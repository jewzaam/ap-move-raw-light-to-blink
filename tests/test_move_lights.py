...
class TestMoveFilesValidation:
    """Tests for move_files input validation."""

    def test_move_files_raises_on_nonexistent_source(self, tmp_path):
        """Test that ValueError is raised for non-existent source directory."""
        source_dir = str(tmp_path / "nonexistent")
        dest_dir = str(tmp_path / "dest")

        with pytest.raises(ValueError, match="Source directory does not exist"):
            move_lights.move_files(source_dir, dest_dir)

    def test_move_files_raises_when_source_is_file(self, tmp_path):
        ...
        with pytest.raises(ValueError, match="Source path is not a directory"):
            move_lights.move_files(str(source_file), dest_dir)


class TestMoveFilesErrorHandling:
    """Tests for move_files error handling."""
    ...
    def test_move_files_continues_on_file_error(
        self,
        mock_delete,
        mock_move_file,
        mock_normalize,
        ...
        """Test that processing continues when a single file move fails."""
        source_dir = str(tmp_path / "source")
        dest_dir = str(tmp_path / "dest")
        os.makedirs(source_dir, exist_ok=True)

        # Mock metadata for two files
        mock_metadata = {
            "file1.fits": {
        ...
        mock_move_file.side_effect = [OSError("Disk full"), None]
        ...
        # Should show 1 file moved (second one succeeded)
        assert "1 LIGHT file(s)" in captured.out
...
class TestMain:
    ...
    @patch("ap_move_lights.move_lights.move_files")
    def test_main_exits_with_code_1_on_validation_error(self, mock_move_files, capsys):
        """Test that main exits with code 1 when ValueError is raised."""
        mock_move_files.side_effect = ValueError("Source directory does not exist")

        with patch("sys.argv", ["move_lights.py", "/nonexistent", "/dest"]):
